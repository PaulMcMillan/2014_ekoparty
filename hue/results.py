...
class QueryResponse(object):
    ...
    # for you.
    OFFSET = 1414000000000000000

    def __init__(self, *args):
        if len(args) < 3:
            print args
        self.path = args[0]
        self.query = self._parse(args[1])
        self.response = map(self._parse, args[2:])
    ...
    def _parse(self, nano_time):
        ...
        if '.' in nano_time:
            ...
            return int(nano_time) - self.OFFSET
...
def read_data(bucket=r'/api/(\w+)/config',
    ...
    if print_summary:
        print "Key Count (Number of loaded samples)"
